...
- 物品栏和 CreativeTab 中通过 `IItemRenderer` 或 block inventory renderer 渲染同一份 baked model。
- 不依赖 GTNH 渲染辅助库，底层只使用 Forge 1.7.10、`Tessellator`、`IIcon`、`GL11`。

当前已经接入两类用法：

- `eco_drive`：有朝向、有 empty/full 两份模型，后续可以接 TileEntity 状态。
- `aluminum_alloy_casing`、`black_tungsten_alloy_casing`：普通静态模型方块。
...
也就是说，JSON 中继续写 `neoecoae:block/...`，但是 `BlockEcoDrive` 和 `BlockModernModel` 注册 icon 时会去掉 `block/`，交给 1.7.10 的 `registerIcon` 解析到 `textures/blocks`。
...
- `display.gui`：用于物品栏 `IItemRenderer` 的 GUI transform。
...
- `EcoModelRenderer.renderInventory`：`IItemRenderer` 入口，使用 JSON 的 `display.gui`。
- `EcoModelRenderer.renderInventoryBlock`：`ISimpleBlockRenderingHandler.renderInventoryBlock` 入口，使用统一默认变换。

注册与接入：

- `ClientProxy.registerRenderers`：分配 render id，加载模型，注册 block renderer 和 item renderer。
- `EcoDriveRenderHandler`：`eco_drive` 的世界/block inventory 渲染。
- `EcoDriveItemRenderer`：`eco_drive` 的 item 渲染。
- `ModernBlockRenderHandler`：普通现代模型方块的世界/block inventory 渲染。
- `ModernBlockItemRenderer`：普通现代模型方块的 item 渲染。

## ECO Drive 接入

`BlockEcoDrive` 是专门为 ECO 存储驱动器准备的方块类。
...
- 当前世界中默认渲染 `EcoDriveVisualState.EMPTY`。
- `EcoDriveModels` 同时加载 `eco_drive_empty` 和 `eco_drive_full`，为后续 TileEntity 状态切换预留。
- 碰撞盒仍保持完整方块，避免凹槽模型影响交互、寻路和邻接判断。

如果放置朝向不对，优先检查：

- `BlockEcoDrive.getFacingMetaFromYaw`
- `ModelFacing.fromMeta`
- `BakedEcoModel.rotateY`
- `BakedEcoModel.rotateDirection`
- `EcoDriveRenderHandler.renderWorldBlock`

如果 CreativeTab/背包方向不对，优先检查：

- `EcoModelRenderer.renderInventory`
- `EcoModelRenderer.renderInventoryBlock`
- `EcoDriveItemRenderer`
- JSON 里的 `display.gui.rotation`
...
3. 在 `NEBlocks` 中用 `modelBlock(id, modelName, textures)` 创建方块。
...
6. 在 `ClientProxy.registerRenderers` 中注册对应 `ModernBlockItemRenderer`。
7. 在 `en_US.lang` 和 `zh_CN.lang` 中补名字。
...
- `renderInventory` 和 `renderInventoryBlock` 使用 `getInventoryShade`。
...
- ECO Drive 还没有接 TileEntity，`full` 模型暂时只是预加载备用。
...
| 现象 | 优先检查 |
| --- | --- |
| 紫黑缺失贴图 | `BlockEcoDrive.TEXTURES` / `BlockModernModel.textureNames` 是否包含 JSON 实际引用的贴图 id |
| 世界模型方向错误 | `getFacingMetaFromYaw`、`ModelFacing.fromMeta`、`BakedEcoModel.rotateY` |
| 物品栏模型方向错误 | JSON `display.gui`、`EcoModelRenderer.renderInventoryBlock` |
| 模型底部太暗 | `getInventoryShade` 或 `getWorldShade` |
| 相邻方块导致凹槽消失 | JSON 的 `cullface` 是否只写在外壳外侧面 |
| 世界能看到，CreativeTab 看不到 | `IItemRenderer` 注册、`handleRenderType`、`renderInventory` |
